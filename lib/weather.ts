...
const silentLogger: WeatherLogger = { info() {}, error() {} };

export class WeatherService {
  // Config resolves per call so a caller reading mutable process.env sees current values.
  constructor(
    private readonly resolveConfig: () => WeatherConfig,
    private readonly log: WeatherLogger = silentLogger,
  ) {}

  private get ambientApiKey() { return this.resolveConfig().ambientApiKey; }
  private get ambientAppKey() { return this.resolveConfig().ambientAppKey; }
  private get ambientMacAddress() { return this.resolveConfig().ambientMacAddress; }
  private get openWeatherApiKey() { return this.resolveConfig().openWeatherApiKey; }
  private get openWeatherCity() { return this.resolveConfig().openWeatherCity || 'Paris'; }
  private get openWeatherCountry() { return this.resolveConfig().openWeatherCountry || 'FR'; }
  ...
  async getCurrentWeather(): Promise<WeatherData> {
    ...
    if (this.isAmbientConfigured()) {
      try {
        ...
        this.log.error('Ambient Weather API failed, falling back to OpenWeatherMap', error);
      }
    }
    ...
    if (this.isOpenWeatherConfigured()) {
      try {
        ...
        this.log.error('OpenWeatherMap API failed', error);
  ...
  private async getAmbientWeather(): Promise<WeatherData | null> {
    ...
    this.log.info('Weather data fetched from Ambient Weather station');
